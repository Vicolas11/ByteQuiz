...
import { UpdCompetition } from "@/interfaces/props.interface";
...
const UpdateQuestionForm = ({ searchParams, data }: UpdCompetition) => {
  ...
  const multiInpValArr = data.questions.map((data, i) => [i, data]);

  const [multiInpVal, setMultiInpVal] = useState<MultiInputValType>(
    Object.fromEntries(multiInpValArr)
  );
  ...
  const newData = Object.values(multiInpVal).map(
    ({ _id, score, time, ...rest }) => ({
      score: +score,
      time: +time,
      ...rest,
  ...
  return (
    <Suspense fallback={<h1 className={styles.loading}>Loading...</h1>}>
      ...
      {showSubmitModal && (
        <QuestionDetailModal data={newData} competeData={data} isUpdate />
      )}
            ...
            Object.values(multiInpVal).map((data, i) => (
              <li key={i}>
                ...
                <div className={styles.txtAreaQuest}>
                  ...
                  <textarea
                    name="question"
                    disabled={!action[data.id || data._id]?.isEdit}
                  ...
                  <div className={styles.btn_group}>
                    <div className={styles.toolTip}>
                      <CustomBtn
                        ...
                        disabled={action[data.id || data._id]?.isEdit}
                        onClick={() => handleAction(data.id || data._id)}
                      />
                    ...
                    <div className={styles.toolTip}>
                      <CustomBtn
                        icon={<FaSave />}
                        xtraStyle={styles.btn}
                        disabled={action[data.id || data._id]?.isSave}
                        onClick={() => handleAction(data.id || data._id)}
                      />
                      <div className={styles.toolTipTxt}>Save Question</div>
                    </div>
                    {data._id && <div className={styles.toolTip}>
                      <CustomBtn
                        icon={<FaTrash />}
                        xtraStyle={styles.btn}
                        onClick={() => removeQuestion(data.id || data._id)}
                      />
