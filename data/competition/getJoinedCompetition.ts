import { JoinedResp, LeaderboardResp } from "@/interfaces/response.interface";
...
export const getJoinedCompetition = async (data: QueryParams) => {
  const token = cookies().get("token")?.value;
  const authToken = verifyToken(token);
  let queryString = `api/competitions/joined`;
  ...
  try {
    ...
    return {
      ...
      data: data.data as Array<JoinedResp>,
      error: null,
